...
The reverse case is the checkout showing less than the branch. The runner reads its own configuration from the checkout — `.claude/**`, `CLAUDE.md` and the other paths it names — so it restores those from the base branch whatever the pull request does to them, and keeps what the pull request wrote under `.claude-pr/`, never executed. Content at the restored path is not the change, and a finding about it describes the checkout. The changed paths among them are handed over per run in `restored-paths.txt` rather than left as a rule to remember, and the change is judged from the copy under `.claude-pr/`.
...
Standards: pr-conventions.md, commit-conventions.md, changelog-conventions.md.
...
## Part `docs` — documentation pages and agent files (every `*.md`, ADRs and standards included, and `.cursorrules`)

Standards: docs-style-guide.md, agents-md-schema.md.
...
## Part `records` — ADRs, the registry and the standards (`docs/adr/**`, `adr-index.md`, `*.link.md`, `standards/**`)

Standards: adr-conventions.md, claims-and-evidence.md.
...
TypeScript doc comments and goldens:
...
A run that judges one part sets `part` to that part's name; the whole routine sets none. The verdict the publication reads for a review in parts is composed from the parts' verdicts and carries `parts`, what each part came to — that is `review_aggregate.py`'s to write, never a run's.

The bracketed severity in front of each line above is that finding's `tag`, one per finding: a run reporting a `[HARD BLOCK]` and a `[DOC DEBT]` writes two findings carrying two tags, not one severity for the verdict. `agent` is `exeris-org-reviewer`, the role that runs this routine rather than this file. `checks_run` names every L1 gate this routine was to read, the ones that had not reported included. Three of them are **mandatory** — `docs-lint`, `commit-lint` and `pr-body-check` — and a verdict that reports any of the three as `not-run` leaves the required check red whatever its `decision` says: a review resting on a gate nobody performed has not been performed either (ADR-087 §B.8). `javadoc-gate` and `tsdoc-gate` are not in that set, because a repository without a gated module runs neither, and a caller whose own gates must also be mandatory names them in `docs-review.yml`'s `mandatory-gates` input.
