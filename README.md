...
- **`agents-ref` defaults to the caller's own `.agents/manifest.yaml` pin.** Naming the bundle commit in both places was a duplication with a silent failure mode: Dependabot moves a `uses:` sha and has no rule for an input, so the two drift and the tooling then disagrees with the vendored tree. The manifest is already the authority on which bundle a repository vendors — `agents_bundle.py verify` fails when they disagree — so reading it is the removal of a second source of truth, not a new one. The reader is scoped to `imports:` rather than taking the first `ref:` in the file, which is the same correction the hook dispatcher needed. An explicit input still wins, for testing a repository against tooling it does not yet pin
